...
def synchronize_signals(
    ecg_signal, ppg_signal, sampling_frequency_ecg, sampling_frequency_ppg
):
    """
    ...
    desired_sampling_frequency = (
        sampling_frequency_ppg  # or any other desired frequency
    )

    # Resample ECG to match the PPG sampling frequency
    number_of_samples_ecg = int(
        len(trimmed_ecg_signal) * desired_sampling_frequency / sampling_frequency_ecg
    )
